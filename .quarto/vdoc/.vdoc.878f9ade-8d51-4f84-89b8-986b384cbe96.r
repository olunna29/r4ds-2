...
births_tibble |>
  ...
  labs( 
    ...
    subtitle = "Weekend Christmases tend to show the steepest drops",
    x = "Year",
...
calendar_resid |>
  ...
  labs(
    ...
    subtitle = "Christmas Day and July 4 show the deepest dips",
    x = "Calendar Date",
...
basketball_tibble |>
  ...
  labs(
    ...
    subtitle = "Higher-ranked recruits are more likely to reach the top tiers",
    x = "Recruit Group",
...
basketball_tibble |>
  ...
  geom_text(
    data = basketball_tibble |>
      filter(name %in% c("LeBron James", "Chris Paul", "Kevin Durant", "Dwight Howard")) |>
      filter(!is.na(rank), !is.na(top_mean_wa)),
    aes(label = name),
    vjust = -0.7,
    show.legend = FALSE
  ...
  labs(
    title = "Recruit Rank and Career Production",
    subtitle = "Top recruits tend to post higher career Win Shares per 48 minutes",
    x = "Recruit Rank",
    y = "Career Win Shares per 48 Minutes",
    color = "Career Tier"
  )
#
#
